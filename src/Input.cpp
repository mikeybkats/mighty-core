...
DetectionResult detectPitchMonophonic(
    const std::vector<float>& samples, size_t offset, int32_t sampleRate)
{
    ...
    const int32_t minLag = static_cast<int32_t>(std::floor(static_cast<double>(sampleRate) / kMaxDetectHz));
    const int32_t maxLag = static_cast<int32_t>(std::ceil(static_cast<double>(sampleRate) / kMinDetectHz));
    ...
    void consumeInput(const float* input, int32_t numFrames) {
        ...
        while (static_cast<int32_t>(inputBuffer.size()) >= kWindowSize) {
            ...
            if (detection.valid && detection.midi != lastMidi) {
                ...
                const double ts =
                    std::chrono::duration<double>(now - listenStartedAt).count();
            ...
            inputBuffer.erase(
                inputBuffer.begin(),
                inputBuffer.begin() + std::min<int32_t>(kHopSize, static_cast<int32_t>(inputBuffer.size())));
        }
    }
};

Input::Input() : impl_(std::make_unique<Impl>()) {}
Input::~Input() { stopListening(); }
...
void Input::startListening() {
    ...
    const oboe::Result openResult = builder

...
void TempoEngine::prepareDeviceTickBuffers(int32_t deviceSampleRate) {
  ...
  for (int i = 0; i < kTickSlotCount; ++i) {
    tickDevice_[i].clear();
    if (tickSources_[i].pcm.empty() || tickSources_[i].sampleRate <= 0) continue;
    if (tickSources_[i].sampleRate == deviceSampleRate) {
      tickDevice_[i] = tickSources_[i].pcm;
    } else {
      tickDevice_[i] =
          resampleLinear(tickSources_[i].pcm, tickSources_[i].sampleRate, deviceSampleRate);
    }
  }
}

void TempoEngine::setTickSoundPcm(int index, std::vector<float> samples, int32_t sourceSampleRate) {
  if (index < 0 || index >= kTickSlotCount) return;
  ...
  if (tickDeviceRatePrepared_ != 0) prepareDeviceTickBuffers(tickDeviceRatePrepared_);
}
...
void TempoEngine::processAudio(float* output, int32_t numFrames) {
  ...
  if (clickPhase_ < activeClickTotalLength_) {
    ...
    if (activeTickSlotForClick_ >= 0) {
      renderClickSample(output, 0, clickPhase_, toRender,
                        tickDevice_[static_cast<size_t>(activeTickSlotForClick_)],
                        activeClickOffbeatSoft_, activeClickPairGainMul_);
    } else {
      renderClickSine(output, 0, clickPhase_, toRender, activeClickOffbeatSoft_,
  ...
  for (int i = 0; i < ticks.count; ++i) {
    ...
    activeClickOffbeatSoft_ = scheduler_.getTwoBeatMeasure() && (beat % 2 == 1);
    activeClickPairGainMul_ =
    ...
    if (idx >= 0 && idx < kTickSlotCount && !tickDevice_[static_cast<size_t>(idx)].empty()) {
      ...
      activeClickTotalLength_ = static_cast<int32_t>(tickDevice_[static_cast<size_t>(idx)].size());
    } else {
    ...
    if (activeTickSlotForClick_ >= 0) {
      renderClickSample(output, tickFrame, 0, toRender,
                        tickDevice_[static_cast<size_t>(activeTickSlotForClick_)],
                        activeClickOffbeatSoft_, activeClickPairGainMul_);
    } else {
      renderClickSine(output, tickFrame, 0, toRender, activeClickOffbeatSoft_,
                      activeClickPairGainMul_);
    }
    clickPhase_ = toRender;
...
void TempoEngine::renderClickSine(float* buf, int32_t frameOffset, int32_t clickSample,
                                  int32_t count, bool softenOffbeat, float pairGainMul) {
  if (clickSample == 0) {
    synthToneZ_ = 0.f;
    offbeatToneZ_ = 0.f;
  }
...
void TempoEngine::renderClickSample(float* buf, int32_t frameOffset, int32_t clickSample,
                                    int32_t count, const std::vector<float>& data,
                                    bool softenOffbeat, float pairGainMul) {
  if (clickSample == 0) offbeatToneZ_ = 0.f;
...
void TempoEngine::close() {
  ...
  for (auto& d : tickDevice_) d.clear();
}

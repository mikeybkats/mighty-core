...
Scheduler::Scheduler(int32_t sampleRate) : sampleRate_(sampleRate) {
}
...
void Scheduler::setSwingFraction(double fraction) {
  swingFraction_.store(std::clamp(fraction, 0.0, 0.5), std::memory_order_relaxed);
}
...
double Scheduler::intervalForEmittedBeat(int beatIndex) const {
  ...
  if (!twoBeatMeasure_.load(std::memory_order_relaxed)) return spb;
  ...
  // Swing redistributes each click pair while keeping the pair total at one
  // straight beat.
  return (beatIndex % 2 == 0) ? halfSpb * (1.0 + swing) : halfSpb * (1.0 - swing);
}

// Advance is called once per audio callback buffer
// Returns tick timing result where the trigger times fall inside the buffer
// timespan
AdvanceResult Scheduler::advance(int32_t numFrames) {
  AdvanceResult result;
  const int64_t bufferEnd = samplePosition_ + numFrames;

  // Emit every beat whose exact trigger timing falls inside
  // [samplePosition_, samplePosition_ + numFrames]
  while (static_cast<int64_t>(nextTickExact_) < bufferEnd &&
         result.count < AdvanceResult::kMaxTicks) {
    auto frameOffset = static_cast<int32_t>(static_cast<int64_t>(nextTickExact_) - samplePosition_);
    ...
    // 0: no tick
    // 1: tick
    // >1: two beat mode, or tempo is very high

...
// BPM is atomic so it can be set from any thread while advance() runs on the
// audio thread.
class Scheduler {
 public:
  // Sample rate is passed in (and may be updated via setSampleRate) because it
  // must match the real output stream: Oboe/miniaudio can run at 44.1 kHz, 48
  // kHz, etc.
  ...
  // When enabled, scheduler runs in double-time (2x click rate vs straight
  // mode). Swing alternates consecutive click intervals while preserving one
  // straight-beat total per pair.
 ...
 private:
  ...
  // Absolute timeline: total frames advanced since reset() (grows across
  // buffers).
  int64_t samplePosition_{0};
  // Next beat boundary as a fractional sample index on the same timeline as
  // samplePosition_.

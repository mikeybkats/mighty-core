...
int main(int argc, char* argv[]) {
  ...
  core.onTick = [&](int beat) { latestBeat.store(beat, std::memory_order_release); };
  ...
  std::cout << "mighty-core-host-check: " << bpm << " BPM for " << seconds
  ...
  while (true) {
    ...
    if (beat != lastPrinted) {
      ...
      std::cout << "  beat " << std::setw(3) << beat << "  t=" << std::setw(6) << ms << " ms\n";
    }
  ...
  std::cout << "\nTotal beats: " << (lastPrinted + 1) << "  (expected ~"
            << static_cast<int>(expectedBeats) << ")\n";
  return 0;
}

...
static const char* kSwingLabels[] = {
    "Straight", "Light (10%)", "Medium (20%)", "Heavy (33%)", "Max (50%)",
};
...
int main() {
  ...
  metro.onTick = [&](int beat) { latestBeat.store(beat, std::memory_order_release); };
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    float elapsed =
        std::chrono::duration<float>(std::chrono::steady_clock::now() - lastBeatTime).count();
    ...
    dl->AddCircle({cx, cy}, 13.0f, ImGui::ColorConvertFloat4ToU32({0.6f, 0.50f, 0.1f, 0.7f}), 0,
                  1.5f);

    // Beat label to the right of the dot
    ImGui::SetCursorScreenPos({cursor.x + 36.0f, cursor.y + 8.0f});
    if (lastBeat >= 0) ImGui::Text("Beat  %d", lastBeat);
    ...
    if (ImGui::Combo("##tick_sound", &tickSoundComboIdx, tickSoundComboGetter, &tickUiCtx,
                     tickComboCount)) {
